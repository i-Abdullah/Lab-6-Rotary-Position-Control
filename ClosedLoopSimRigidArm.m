...
SettleTime(index,1) = SystemInfo.SettlingTime; % each row represent 1 Kd value, each column Kp
OvershootValue(index,1) = SystemInfo.Overshoot;  % each row represent 1 Kd value, each column Kp
...
Settle_table =  [ SettleTime' ];

Overshoot_table =  [ OvershootValue' ];


Table_results = table(Kd_table',Kp_table',Settle_table',Overshoot_table',...
    'VariableNames',{'Kd','Kp','Settle_time','Overshoot_percent'})
